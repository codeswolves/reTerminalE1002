...
- **任务流程跟踪树**：可视化每个任务的推进节点、耗时、负责人，支持交互式增删改节点，可 📌 **置顶**当前重点任务
- **项目管理（DAG）**：多项目索引 + 从起点到结束节点的有向无环图，支持里程碑依赖、**多路汇聚**、进度跟踪与页面上直接编辑
- **时间管理四象限**：按重要性 × 紧迫性归类任务（A/B/C/D），对比各象限预估工时占比与目标区间（A 20-25% / B 65-80% / C ≤15% / D 0）并给出偏差诊断；支持拖拽归类、工时录入、7 问自检，已完成任务可做后评估（卡点归类）；含周时间安排（拖拽排期 + 周自评）与仪表盘（平均完成时间、每周完成数、产出成果、估时偏差、分类投入占比、12 周趋势）
...
**时间预算**：四象限比例的分母是**净可安排时间** —— `8h/天 − 1h 机动 = 7h/天`，按 5 天计为 `35h/周`。
那 1 小时机动专门用于突发事项，**不参与象限分配**（算进分母等于预先给"未知"分配了确定的时间）。
页面顶部可临时调整天数与机动时长，只影响小时换算，不落盘。
...
- **工时录入**：点卡片上的 `⏱` / "未估算" 填写预估剩余工时（`estimate_h`）与实际净投入（`actual_h`）。后者用于估时校准 —— 知道"实际是估算的多少倍"比"估得更准"更有用
- **7 问自检**：待归类任务可点"7 问自检"，按"价值 × 能否推卸"推导象限；四问全否且拒绝无成本时，建议**直接拒绝、不入清单**（D 象限的正解是入口拒绝）
- **任务后评估**：已完成任务可打开后评估 —— 完成时间、总历时、估时偏差、最长空档、推进时间轴全部自动算出，只在检测到异常时才定向提问（带日期锚点 + 预填选项），人只需点选卡点类型

- **周时间安排**：拖动任务卡片到时间表即可排期（按 `estimate_h` 自动铺开，遇到午休或已占用时段会跳过并继续往后排）；支持查看上一周 / 下一周，**到新的一周自动切换，旧周数据保留**
- **周自评**：每周可写一段自我评述；保存时会记下当时的**两类快照**（已排工时/时段数、本周已完成数与净投入），避免日后只能用今天的数字去解释当时的结论
- **仪表盘**：平均完成时间（同时给中位数，并单独列出"当天完成"与"跨天任务"两个口径）、本周完成数、产出成果、**估时偏差系数**（`实际 ÷ 预估`，即 §2.8 要的校准数）、各类任务的时间投入占比（实际 vs 计划对照）、近 12 周完成趋势

卡点类型（`需求不清 / 依赖他人 / 技术难点 / 返工 / 被打断 / 精力不足 / 外部阻塞 / 估算失误`）定义在 `src/generators/meta.py`。
归类而非自由文本，是为了能统计"最常因为什么卡住"并反哺配额调整。

**相关接口**：`/api/set_quadrant`（象限）、`/api/set_estimate`、`/api/set_actual`（工时）、`/api/complete_task`（完成任务）、`/api/week_plan`（周排期，支持 `?week=`）、`/api/week_review`（周自评）。

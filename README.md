...
**相关接口**：`/api/set_quadrant`（象限）、`/api/set_estimate`、`/api/set_actual`（工时）、`/api/set_deliverable`（预期成果）、`/api/complete_task`（完成任务）、`/api/week_plan`（周排期，支持 `?week=`）、`/api/week_review`（周自评）。

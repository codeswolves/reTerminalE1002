...
- **计划任务（精确排一段）**：点周表工具栏的「＋ 计划任务」给**已有任务**指定"某天 × 某时段"，不用拖。它补的是现有路径都缺的那一格 —— 拖拽按剩余工作量自动铺开、任务编排会替换整周、时段弹窗得先有时段，**只有这里能直接填起止**。开始/时长/结束三者联动（改任一，另两个跟上），可按"做一小时半"或"做到五点半"两种说法输入；可顺手改该任务的**预估工时**；预览行与时段弹窗同一套口径。**会拦住重叠时段和跨午休的时段**（拖拽与自动编排本来就是自动找空位/跳过午休，只有这里是人工指定，所以只有这里需要拦）。与「临时任务」的区别：那个新建一条时间记录（已发生的事），这个是给已有任务排期

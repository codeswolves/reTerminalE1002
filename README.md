...
| `POST /api/add_temp_task` | 记一笔临时（突发）任务：建任务（`temp: true`）+ 排进周表时段，**一次写完**，入参 `{"name":"临时会议","date":"2026/09/20","from":15,"to":17}` |
...
- **临时（突发）任务**：点「＋ 临时任务」记一笔突发事项并落到时间表（虚线框 + 琥珀色，半小时的短块自动切换紧凑布局）。它**只做时间记录、不进待办池** —— 不进待归类、不计入象限占比、不进仪表盘。周表底部汇总为 `机动：已用 Xh / 额度 Yh`，超支标红
...
**相关接口**：`/api/set_quadrant`（象限）、`/api/set_estimate`、`/api/set_actual`（工时）、`/api/set_deliverable`（预期成果）、`/api/complete_task`（完成任务）、`/api/week_plan`（周排期，支持 `?week=`）、`/api/week_review`（周自评）、`/api/week_buffer`（每周机动额度）、`/api/add_temp_task`（记一笔临时任务，同时建任务 + 排时段）。
...
- `weeks`：该周的时段。`day` 为 `1`（周一）~ `7`（周日），`from`/`to` 是 24 进制小时，只允许落在 `08:30–22:30` 并对齐到半小时

...
| `POST /api/add_task` | 添加新任务（带 `date` + `slots:[{from,to}]` 时**同时排进那一周的若干时段**，一次写完；也认单个 `from`/`to` 写法） |
...
- **临时（突发）任务**：点「＋ 临时任务」记一笔突发事项并落到时间表（虚线框 + 琥珀色，矮块自动切换紧凑布局）。时间**可直接输入**：**起点精确到分钟、时长按 10 分钟计**（`21:25–21:35` 这种也能录，不必凑整到 `:10`）；可标**状态**（已完成 / 进行中 / 未开始，默认已完成）、**预估工时**（留空则按 时长 算）和**机动来源**（会议 / 他人求助 / 行政事务 / 计划外沟通 / 其他，**可留空**）。它**只做时间记录、不进待办池** —— 不进待归类、不计入象限占比、不进仪表盘；但**未完成的会出现在待安排池**（带「临时」角标），因为突发的活常常一笔排不完，需要能继续占时间。周表底部汇总为 `机动：已用 X / 额度 Y`（不足 1 小时按分钟显示），超支标红，下一行按来源列出**构成**（如 `他人求助 2h · 会议 1h · 未标注 1h`）；周自评弹窗里同样显示这份构成，便于复盘。**在周表里点它的时段块就能收工**：``✅ 完成`` 会按 结束−开始 自动补上实际投入；**同一处也能补标/改标机动来源**（临时任务默认已完成、会离开待安排池，不在这里留个入口就再也补不上）
- **录入新任务（建任务 + 精确排一段）**：点周表工具栏的「＋ 新任务」**填一个新的任务名**，同时指定"某天 × 某时段"，一次落盘。它补的是现有路径都缺的那一格 —— 拖拽只能拖已有卡片且按剩余工作量自动铺开、任务编排会替换整周、时段弹窗得先有时段、任务清单页只能建不能排，**只有这里能"建新任务 + 直接填起止"**。开始/时长/结束三者联动（改任一，另两个跟上），可按"做一小时半"或"做到五点半"两种说法输入；四象限**可不选**（默认未归类，之后在象限里归）；预估工时**留空则按本次时长兜底**。**跨午休不用录两次**：填 `09:00–17:00` 会自动排成 `09:00–11:30` + `13:30–17:00` 两段（预览行显示的是**实际能排的 6h**，而不是 8h 的墙钟时长；整段都落在午休里才真的拒掉）。**重叠仍会拦**（逐段查）。与「临时任务」的分工：那个录"已经发生的突发"（默认已完成、不进待办池、不参与象限统计、消耗算机动），这个录"计划要做的事"（进象限与待安排池、从 0% 开始、参与统计）
...
- `weeks`：该周的时段。`day` 为 `1`（周一）~ `7`（周日），`from`/`to` 是 24 进制小时，只允许落在 `08:30–22:30` 并对齐到**整分钟**；**时长**必须是 10 分钟（`GRID_MIN`）的整数倍，但**起点不限**（所以 `21:25–21:35` 这种也能录 —— 真实的事不按 `:10` 发生）

...
- **时间管理四象限**：按重要性 × 紧迫性归类任务（A/B/C/D），对比各象限预估工时占比与目标区间（A 20-25% / B 65-80% / C ≤15% / D 0）并给出偏差诊断；支持拖拽归类、工时录入、7 问自检，已完成任务可做后评估（卡点归类）
...
- **移动端适配**：四个交互页面各带 `@media (max-width: 640px)`（布局）与 `@media (hover: none)`（触屏点击区）两组规则，写在各自 `</style>` 之前；仪表盘为墨水屏固定 800×480 不参与适配。详见 [移动端适配设计文档](docs/design/mobile-responsive.md)

...
## 「刷新几次才显示」的竞态：`render()` 漏了一个重绘（2026-09-21 踩过）

**现象**（NAS 部署后更明显）：周表里刚建的临时任务**有时不显示**，多刷几次才出来。
但底部那行 `机动：已用 2h / 额度 5h` 却是**对的**。

**这个"统计对、块没有"的组合就是它的指纹** —— 两者依赖同一份数据，只可能是一个重绘了、一个没重绘。

**根因**：`init()` 里两个请求是**并发**的，各自的回调只重绘一部分：

| 请求 | 成功后调 | 会重绘周表块吗 |
|---|---|---|
| `loadData()`（拉任务） | `render()` | ❌ **不会**（漏了 `renderWeek()`）|
| `loadWeekPlan()`（拉周计划） | `renderWeek()` | ✅ 会 |

而时段块要靠 `byNo(s.no)` 回**任务池**取名字，取不到就 `return ''` 整块跳过。于是：

- `loadData` 先返回 → 画块时任务池已是最新 → ✅ 显示
- **`loadWeekPlan` 先返回** → 任务池还是**页面生成时的快照** → ❌ 新任务找不到 → 块不渲染

本地两个请求都在毫秒级，顺序几乎固定；**NAS 上延迟抖动大，谁先返回变得随机**，
于是表现为"有时候"，极易被误判成网络问题（"是不是网络差？"）。

**教训**：`render()` 的语义是"把页面重绘一遍"，**漏掉任何一个渲染函数都是隐患**。
新增渲染函数时，要检查它依赖的数据有没有异步来源；有的话，必须能被 `render()` 覆盖到。

**通用判据**：页面上出现"**同一份数据的两个视图，一个对一个不对**"，
先查这两个视图分别由哪个函数重绘、那条链在不在同一个回调里 —— 通常就是漏了一步。

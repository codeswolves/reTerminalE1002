...
约定:
    - 一个任务可以在一周内出现多次(多个时段)
    - from / to 是小时(24 进制), 只允许落在 [DAY_START, DAY_END], 并对齐到**整分钟**;
      时长必须是 GRID_MIN(10 分钟) 的整数倍(起点不限: 21:25 的会就记 21:25)
...
#
# **起点不要求落在刻度上**(2026-09-22 起改): 原先起止都必须是 10 分钟的整数倍, 于是
# "21:25 那个会"只能记成 21:20 或 21:30 —— 凭空差几分钟。而真实的事不按 :10 发生:
# 这个网格的意义是"别记成 9:07 这种精度", 不是"事情都发生在整十分"。
GRID_MIN = 10                       # 时长粒度(分钟)
...
def snap_minute(h):
    ...
    对齐不是为了"限制精度"(时间本来就是按分钟填的), 而是抹掉浮点噪声 ——
    分钟换成小时普遍没有精确表示(21:25 = 21.416666666666668), 不对齐的话
    同一个时刻在反复读写之后会漂成略有差别的值。

    **刻意不对齐到 GRID_MIN 网格**: 那样 21:25 会被静默挪成 21:30, 用户看到的是
    "保存成功、却排在了别的时间", 只会以为是自己记错了(见 GRID_MIN 的注释)。
    """
    return round(h * 60) / 60


def clean_slots(raw):
    """清洗 slots: 丢弃非法项, 对齐到整分钟, 并把时间范围夹到工作时间窗内。

    注意这里是**兜底**清洗(含旧数据的浮点噪声)。用户填的时长是否是 GRID_MIN 的整数倍,
    由接口层显式校验并**明确拒绝** —— 静默改掉时长会表现为"保存成功、却和填的不一样"。
    """
    out = []
    ...
    for s in raw:
        ...
        if not 1 <= day <= 7:
            continue
        h_from = snap_minute(max(DAY_START, min(DAY_END - MIN_SLOT_HOURS, h_from)))
        h_to = snap_minute(max(h_from + MIN_SLOT_HOURS, min(DAY_END, h_to)))

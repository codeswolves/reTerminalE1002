...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"  # src/setup/ 目录
...
# 创建示例 CSV（如果不存在）
for f in weight.csv fitness.csv; do
...
CRON_JOB="0 7 * * * cd ${PROJECT_ROOT} && python3 src/pipeline/run_daily.py >> /tmp/dashboard_cron.log 2>&1"
...
echo "║  安装完成！                                      ║"
echo "║                                                  ║"
echo "║  手动运行: python3 src/pipeline/run_daily.py       ║"
echo "║  查看定时: crontab -l                            ║"
...
echo "║    data/task_flows.json - 任务流程数据            ║"
echo "╚══════════════════════════════════════════════════╝"
